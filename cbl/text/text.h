 ...
 *
 *  struct @c text_t implements a text that is an alternative representation of character strings.
 *  The C representation of strings using a terminating null character has some drawbacks:
 ...
 *  For example, suppose concatenating a string to another and those strings are not immutable. To
 *  determine the byte into which the former string is to be put, one has to compute the length of
 *  the later string, which requires to scan all characters in it. Besides, one has to somehow
 *  allocate storage to contain the result because there is no way for two different strings to
 *  share the same sequence of characters in the storage. Jobs like scanning strings and allocation
 *  of new storage can be avoided by getting rid of the terminating null character and keeping the
 *  length of a string separately.
 *
 *  Because a null character does not play a role of terminating strings anymore, there is no need
 *  to treat it specially. Therefore, a text represented by @c text_t is allowed to contain a null
 *  character anywhere in it, and other @c text_t functions never treat it in a special way.
 *  Nevertheless, be warned that, if a text which has a null character embedded in it is converted
 ...
 *  design approach simplifies an implementation since they need to allocate no descriptor for a
 *  text. The size of @c text_t being not so big, passing its value would cause no big penalty on
 *  performance.
 */
 ...
 *  text space and restoring the space to make it have the top remembered in the @c text_save_t
 *  object as the current top. For more details, see struct @c text_save_t, struct @c chunk,
...
/*! @brief    accesses a character in a text with a position.
 *
