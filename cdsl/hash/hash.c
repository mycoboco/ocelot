 ...
 * that of struct @c hash_t, but plus @c len. Then the pointer @c str points to the extra space.
 * This is dipicted below.
 *
 * @code
 *                          |--------- len ---------|
 *     +------+------+------+-----------------------+
 ...
 * index. The numbers are generated by the standard's rand(). This scatter table is need to be
 * revised when the number of characters in an implementation is greater than 256. Because the hash
 * numbers are calculated by the library using @c scatter, the size of @c bucket can have a less
 ...
 *  and which is what hashing is for.
 *
 ...
 *
 *  @warning    It leads to an unpredictable result to modify a hash string returned by the library.
 *
 ...
 *  is chosen for speeding hash_new() up with @c scatter that helps to restrain conflicts; profiling
