 ...
 * @c binding, even if such a violation rarely happens in practice. Putting a dummy member of the
 ...
 * converts a pointer value to an unsigned integer and cuts off its two low-order bits because they
 * are verly likely to be zero due to the alignement restriction. It returns an integer resulting
 * from converting to a possibly narrow type.
 *
 ...
 *  generated by the Hash Library. An example follows:
 *
 *  @code
 ...
 *
 *  table_put() replaces an existing value for a key with a new one and returns the previous value.
 *  If there is no existing one, the value is saved for the key and returns a null pointer.
 *
 ...
 *
 *  table_remove() gets rid of a key-value pair for a key from a table. Note that table_remove()
 *  does not deallocate any storage for the pair to remove, which must be done by an user.
 *
 *  Possible exceptions: assert_exceptfail
 *
 *  Unchecked errors: foreign data structure given for @p table
 ...
 *  Unchecked errors: foreign data structure given for @p table
 *
 *  @warning    The size of an array generated from an empty table cannot be zero, since there is
